...
const checkRule = {
  ...
  password: t => /^[a-zA-Z0-9_-]{6,12}$/.test(t)
}
...
$('userPass').onchange = e => {
  return checkInput(e.target, 'password', '密码为6~12位数字、大小写字母、中划线、下划线')
}

$('userPassAgain').onchange = e => {
  if (e.target.value === $('userPass').value) {
    e.target.className = 'rightInput'
    notice.hide()
    return true
  } else {
    e.target.className = 'errorInput'
    notice.show('两次输入密码不一致')
    return false
  }
}
...
$('submit').onclick = e => {
  ...
  if (!$('userPass').onchange({ target: $('userPass') })) {
    $('userPass').focus()
    return
  }
  if ($('userPassAgain').value !== $('userPass').value) {
    $('userPassAgain').className = 'errorInput'
    notice.show('两次输入密码不一致')
    $('userPassAgain').focus()
    return
  }
